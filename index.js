...
function replaceText(word, replacement, text) {
  ...
  //const reg1 = /word/g;
  text = text.replaceAll(reg, replacement);
  return text;
...
function countEmails(str) {
  const reg = RegExp(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g);
...
function checkRegexMethods(regex) {
  let metodsArr = [];
  if (regex.dotAll) {
    metodsArr.push(`dotAll`);
  }
  if (regex.multiline) {
    metodsArr.push(`multiline`);
  }
  if (regex.sticky) {
    metodsArr.push(`sticky`);
  }
  return metodsArr;
